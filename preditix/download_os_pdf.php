...
try {
    ...
    // Buscar o PDF da OS
    $sql = "SELECT pdf, numero_os FROM ordens_servico WHERE id = ? AND pdf IS NOT NULL";
    $resultado = $db->query($sql, [$os_id]);
    
    if (!empty($resultado) && !empty($resultado[0]['pdf'])) {
        $pdf_conteudo = $resultado[0]['pdf'];
        $numero_os = $resultado[0]['numero_os'];
        
        // Configurar headers para download
        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="OS_' . $numero_os . '.pdf"');
